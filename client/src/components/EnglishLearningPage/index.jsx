...
function LearnEnglishPage() {
  ...
  const handleUserResponse = (selectedOption) => {
    ...
    if (isCorrect) {
      ...
      setRatingChangeMessage(`Congratulations! Your rating has increased by ${currentQuestion.IncreaseAmount}. New Rating: ${newRating}`);
      ...
      setUserRating(newRating);
      updateRating( newRating);
      setRatingChangeMessage(`Oops! Wrong answer. Your rating has decreased by ${currentQuestion.decreaseAmount}. New Rating: ${newRating}`);
      fetchQuestion( userId, newRating);
  ...
  const renderQuestion = () => {
    ...
    return (
      <div>
        ...
        <h1>Question:</h1>
        {ratingChangeMessage && <p>{ratingChangeMessage}</p>}
        {question ? (
          <div>
            <p>{question.text}</p>
            ...
            <table className={styles.optionTable}>
              <tbody>
                {question.options.map((option, index) => (
                  <tr key={index}>
                    <td>
                      <button className={styles.optionButton} onClick={() => handleUserResponse(option)}>
                        {option}
                      </button>
                    </td>
                  </tr>
